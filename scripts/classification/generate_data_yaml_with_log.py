...
def build_data(
    ...
    data = {"train": {mc: [] for mc in all_main_classes},
            "validation": {mc: [] for mc in all_main_classes}}
...
def compute_summary(data: Dict[str, Dict[str, List[str]]]):
    totals_by_split = {split: sum(len(v) for v in data[split].values()) for split in ("train", "validation")}
...
def main():
    ...
    parser.add_argument("--input_folders", required=True, type=Path)
    ...
    subclass_to_main, _ = reverse_subclass_map(class_sep)

    data = build_data(
        base_dir=args.base_dir,
        split_to_folders={"train": input_cfg.get("train", []), "validation": input_cfg.get("validation", [])},
    ...
    with args.out.open("w", encoding="utf-8") as f:
        yaml.dump({"train": data["train"], "validation": data["validation"]}, f, sort_keys=False, allow_unicode=True)
